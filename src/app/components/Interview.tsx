...
export default function Interview() {
  ...
  const startQuestions = () => {
    ...
    setScreen("question"); setCurrentQ(0); goToQuestion(0);
  };
  ...
  const goToContact = () => {
    window.speechSynthesis?.cancel(); setIsSpeaking(false);
    setScreen("contact"); sayText("ありがとう！最後に、連絡先を教えてもらえるかな？");
  };

  const submitContact = () => {
    if (!nickname.trim() || !contact.trim()) return;
    setScreen("complete"); sayText(CONFIG.completeMessage);
  };
  ...
  if (screen === "title") {
    return (
      <div className="relative mx-auto h-dvh w-full max-w-[430px] overflow-hidden select-none flex flex-col">
        ...
        <div className="absolute top-[10%] left-1/2 -translate-x-1/2 w-72 h-72 bg-white/25 rounded-full blur-3xl" />

        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-10">
          {/* タイトルカード */}
          <div className="bg-white/20 backdrop-blur-sm rounded-3xl px-10 py-8 border border-white/30 shadow-lg mb-10">
            <h1 className="text-5xl font-black text-[#1B5E20] tracking-wider leading-tight text-center">AI面談</h1>
            <div className="mt-4 flex items-center justify-center gap-3">
              <span className="h-px flex-1 bg-[#2E7D32]/30" />
              <span className="text-[#2E7D32] text-xs font-bold tracking-[0.25em]">INTERVIEW</span>
              <span className="h-px flex-1 bg-[#2E7D32]/30" />
            </div>
          </div>

          {/* 説明 */}
          <p className="text-[#2E7D32] text-lg font-bold mb-4 text-center">履歴書なし・スマホで10分</p>
          <div className="flex justify-center gap-5 text-[#2E7D32]/70 text-xs mb-10">
          ...
          {/* ボタン */}
          <button onClick={startInterview} className="vn-btn vn-btn-accent w-full max-w-[300px] py-5 text-xl animate-gentle-pulse tracking-[0.15em] rounded-2xl">
            はじめる
          </button>
          <p className="mt-8 text-[#2E7D32]/40 text-[11px]">AI面接官「翔平」が8つの質問をします</p>
        </div>

        <div className="absolute bottom-0 left-0 right-0 h-[10%] bg-gradient-to-t from-[#81C784] to-transparent" />
      </div>
    );
  }
