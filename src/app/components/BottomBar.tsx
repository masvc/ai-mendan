...
export default function BottomBar({ onBack, onRecord, onStop, onNext, onRestart, isRecording, canGoBack, canGoNext, disabled, mode, recordLabel }: Props) {
    ...
    return (
      <div className="bg-white/95 backdrop-blur-md rounded-2xl">
        <div className="px-6 py-8 flex justify-around items-center">
          <button onClick={onRestart} className={`${btnBase} w-[85px] h-[85px] rounded-2xl border-2 border-slate-200 text-slate-400 shadow-[0_2px_0_#e2e8f0] active:shadow-none`}>
  ...
  return (
    <div className="bg-white/95 backdrop-blur-md rounded-2xl">
      <div className="px-6 py-8 flex justify-around items-center">
        <button onClick={onBack} disabled={!canGoBack || disabled} className={`${btnBase} w-[85px] h-[85px] rounded-2xl text-slate-400 disabled:opacity-30 border-2 border-slate-200 shadow-[0_2px_0_#e2e8f0] active:shadow-none`}>
        ...
        {!isRecording ? (
          <button onClick={onRecord} disabled={disabled} className={`${btnBase} w-[110px] h-[110px] rounded-3xl bg-red-600 text-white disabled:opacity-30 shadow-[0_3px_0_#991b1b] active:shadow-none`}>
          ...
          <button onClick={onStop} className={`${btnBase} w-[110px] h-[110px] rounded-3xl bg-slate-700 text-white shadow-[0_3px_0_#334155] active:shadow-none animate-rec-pulse`}>
            <span className="w-6 h-6 bg-red-400 rounded-md" />
            <span className="text-base font-bold mt-1">停止</span>
          </button>
        )}

        <button onClick={onNext} disabled={!canGoNext || disabled} className={`${btnBase} w-[85px] h-[85px] rounded-2xl bg-[#1e293b] text-white disabled:opacity-30 shadow-[0_2px_0_#0f172a] active:shadow-none`}>
